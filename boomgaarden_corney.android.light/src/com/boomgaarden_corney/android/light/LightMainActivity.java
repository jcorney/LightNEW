package com.boomgaarden_corney.android.light;
...
public class LightMainActivity extends Activity implements
		SensorEventListener {

	private final String DEBUG_TAG = "DEBUG_LIGHT";
	private final String SERVER_URL = "http://54.86.68.241/light/test.php";
	...
	private float lightAccuracy;
	private float lightValue0;
	private float lightMaxRange = 0;
	private float lightPower = 0;
	private float lightResolution = 0;
	private int  lightSensorType;
	private int numLightChanges = 0;
	private int lightVersion = 0;
	private int lightHashCode = 0;
	...
	private List<NameValuePair> paramsLight = new ArrayList<NameValuePair>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// Setup Light Manager and Provider
	...
	/* Request Light updates at startup */
	@Override
	...
	/* Remove the Lightlistener updates when Activity is paused */
	@Override
	...
	public void onSensorChanged(SensorEvent event) {
		if (mLight != null) {
			if ((event.sensor.getType() == mLight.getType()) && numLightChanges < 10) {
	...
	private String sendHttpRequest(String myURL, String postParameters)
		...
		} else if (postParameters.equals("LIGHT")) {
			writer.write(buildPostRequest(paramsLight));
			paramsLight = new ArrayList<NameValuePair>();
	...
	private class SendHttpRequestTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			// request to send - i.e. whether to send Device or Light
			// parameters.
			try {
	...
	private void setLightData(SensorEvent light) {
		lightAccuracy = light.accuracy;
		lightSensorType = light.sensor.getType();
		lightTimeStamp = light.timestamp;
		lightValue0 = light.values[0];
		lightHashCode = light.hashCode();

		paramsLight.add(new BasicNameValuePair("Light Update Count",
				String.valueOf(numLightChanges)));
		paramsLight.add(new BasicNameValuePair("Accuracy", String
				.valueOf(lightAccuracy)));
		paramsLight.add(new BasicNameValuePair("Sensor Type", String
				.valueOf(lightSensorType)));
		paramsLight.add(new BasicNameValuePair("Time Stamp", String
				.valueOf(lightTimeStamp)));
		paramsLight.add(new BasicNameValuePair(
				"Value 0 Ambient light level in SI lux units", String
						.valueOf(lightValue0)));
		paramsLight.add(new BasicNameValuePair(
				"Hash Code Value", String
						.valueOf(lightHashCode)));
	}
	
	private void setSensorData() {
		lightMaxRange = mLight.getMaximumRange();
		lightPower = mLight.getPower();
		lightResolution = mLight.getResolution();
		lightVendor = mLight.getVendor();
		lightVersion = mLight.getVersion();		
		
		paramsSensor.add(new BasicNameValuePair("Max Range", String
						.valueOf(lightMaxRange)));
		paramsSensor.add(new BasicNameValuePair("Power", String
				.valueOf(lightPower)));
		paramsSensor.add(new BasicNameValuePair("Resolution", String
				.valueOf(lightResolution)));
		paramsSensor.add(new BasicNameValuePair("Vendor", String
				.valueOf(lightVendor)));
		paramsSensor.add(new BasicNameValuePair("Version", String
				.valueOf(lightVersion)));
	}
	...
	private void showLightData() {
		...
		results.append("Light Update Count: "
				+ String.valueOf(numLightChanges) + "\n");
		results.append("Light Accuracy: " + String.valueOf(lightAccuracy) + "\n");
		results.append("Light Sensor Type: " + String.valueOf(lightSensorType) + "\n");
		results.append("Light Time Stamp: " + String.valueOf(lightTimeStamp) + "\n");
		results.append("Light Vaule 0 (X axis): " + String.valueOf(lightValue0) + "\n");
		results.append("Light Hash Code " + String.valueOf(lightHashCode) + "\n");
	...
	private void showSensorData() {
		...
		results.append("Max Range: " + String.valueOf(lightMaxRange) + "\n");
		results.append("Power: " + String.valueOf(lightPower) + "\n");
		results.append("Resolution: " + String.valueOf(lightResolution) + "\n");
		results.append("Vendor: " + String.valueOf(lightVendor) + "\n");
		results.append("Version: " + String.valueOf(lightVersion) + "\n");
	...
	private void sendLightData() {
		...
		if (networkInfo != null && networkInfo.isConnected()) {
			// Send HTTP POST request to server which will include POST
			// parameters with Light info
			new SendHttpRequestTask().execute(SERVER_URL, "LIGHT");
		} else {
			setErrorMsg("No Network Connectivity");
			showErrorMsg();
		}
